...
const Product = () => {
  ...
  return <div className='py-10 mt-32'>
    {active ? <Loading/> : 
        ...
        <div className='bg-white'>
          <div className='py-5 px-10'>
            <div className='relative'>
              <img  src={select === "" ? productsData.thumbnail : productsData?.images[select]} className='w-full h-[320px]'  alt='single-img' />
              <p className='absolute text-gray-100 top-5 left-0 py-2 px-5 bg-orange-500'>{productsData.brand}</p>
            </div>
              ...
              {productsData?.images?.map((img,ind)=>(
                <div key={ind} className='mb-3 cursor-pointer shadow p-3'>
                  <img onMouseOver={()=>{
                    ...
                    setselect(ind)
                  }} src={img} className='w-32 h-20' alt='img' />
                </div>
              ))}
            </div>
          </div>
          <div className='px-5 lg:px-10 py-10'>
            <div>
              ...
              <div className='flex'>
              <p className='text-orange-500 text-[14px] border-r-[3px] border-gray-300 pr-5 font-bold'>Rating : <span className='text-gray-500'> {productsData.rating}</span></p>
              <p className='text-orange-500 text-[14px] border-r-[3px] border-gray-300 px-5  font-bold'>category :  <span className='text-gray-500'>{productsData.category}</span></p>
              <p className='text-orange-500 text-[14px] px-5 font-bold'>brand : <span className='text-gray-500'> {productsData.brand}</span></p>
              </div>
              <div className='p-5'>
                <p className='text-gray-400 flex gap-3'><span className='line-through '> ${productsData?.price} </span> (inclusive of all taxes)</p>
                <p className='text-orange-500 mt-2 font-bold text-xl flex gap-2'>${(productsData.price - (productsData.price * (productsData.discountPercentage / 100))).toFixed()} <span className='bg-orange-500 h-fit py-[1px] px-2 text-sm font-normal rounded-md text-gray-100'>{Math.floor(productsData.discountPercentage)}% off</span></p>
              </div>
            </div>
      ...
      {isActive ? <div className='bg-teal-500 py-3 rounded-full w-96 text-center text-gray-100 fixed top-10 z-[9999] left-[50%] translate-x-[-50%]'>
